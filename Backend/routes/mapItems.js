...
router.post('/map/items', async (req, res) => {
  ...
  try {
    ...
    await logAudit('admin', 'CREATE', 'map_item', id, null, result.rows[0]);
...
// ── 역량맵 아이템 수정 ──────────────────────────────────────
router.put('/map/items/:id', async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    await logAudit('admin', 'UPDATE', 'map_item', id, before.rows[0], result.rows[0]);
...
// ── 역량맵 아이템 삭제 ──────────────────────────────────────
router.delete('/map/items/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const before = await pool.query('SELECT * FROM map_items WHERE id=$1', [id]);
    if (!before.rows.length) return res.status(404).json({ error: '아이템 없음' });
    ...
    await logAudit('admin', 'DELETE', 'map_item', id, before.rows[0], null);
    res.json({ ok: true });
  } catch (err) {
